...
router.post('/register', async (req: Request, res: Response) => {
  try {
    ...
    if (exists.rows.length > 0) return res.status(400).json({ error: 'Cet email est déjà utilisé' });
    const hash = await bcrypt.hash(data.password, 12);
    const result = await pool.query(
      'INSERT INTO users (email, pin_hash, name, language) VALUES ($1,$2,$3,$4) RETURNING id',
      [data.email, hash, data.name, data.language || 'fr']
    );
    res.status(201).json({
...
router.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query('SELECT id, email, pin_hash, name, language, role FROM users WHERE email = $1', [data.email]);
    if (result.rows.length === 0) return res.status(400).json({ error: 'Email ou mot de passe incorrect' });
    const user = result.rows[0];
    const valid = await bcrypt.compare(data.password, user.pin_hash);
...
router.get('/me', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const result = await pool.query('SELECT id, email, name, language, role, created_at FROM users WHERE id = $1', [req.userId]);
