...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    for (const time of data.times) {
      await client.query(
        'INSERT INTO medication_times (medication_id, time) VALUES ($1,$2)',
        [medId, time]
      );
    }
    ...
    const timesResult = await client.query(
      'SELECT time FROM medication_times WHERE medication_id = $1 ORDER BY time',
      [medId]
    );
    ...
    res.status(201).json({
      medication: {
        ...medResult.rows[0],
        times: timesResult.rows.map((t: any) => t.time),
      },
    });
  } catch (error) {
