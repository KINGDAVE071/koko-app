...
router.put('/settings', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (privacy_policy !== undefined) {
      await pool.query(`INSERT INTO settings (key, value) VALUES ('privacy_policy', $1) ON CONFLICT (key) DO UPDATE SET value = $1, updated_at = NOW()`, [privacy_policy]);
    }
    if (terms_of_service !== undefined) {
      await pool.query(`INSERT INTO settings (key, value) VALUES ('terms_of_service', $1) ON CONFLICT (key) DO UPDATE SET value = $1, updated_at = NOW()`, [terms_of_service]);
    }
    if (contact_email !== undefined) {
      await pool.query(`INSERT INTO settings (key, value) VALUES ('contact_email', $1) ON CONFLICT (key) DO UPDATE SET value = $1, updated_at = NOW()`, [contact_email]);
    }
