...
const medicationSchema = zod_1.z.object({
    ...
    times: zod_1.z.array(zod_1.z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/)).min(1, 'Au moins une heure de prise est requise'),
});
router.post('/', auth_1.authMiddleware, async (req, res) => {
    ...
    try {
       ...
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`, [req.userId, data.name, data.dosage || null, data.frequency, data.start_date, data.end_date || null]);
        ...
        res.status(201).json({ medication: { ...medResult.rows[0], times: times.rows.map((t) => t.time) } });
    }
    catch (error) {
...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    ...
    for (const med of medications.rows) {
        ...
        result.push({
            ...
            logs: logs.rows.reduce((acc, l) => { acc[l.time] = l.taken; return acc; }, {}),
        });
    }
