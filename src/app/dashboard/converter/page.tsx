...
export default function ConverterPage() {
  ...
  const handleMargin = async () => {
    const cost = parseFloat(prompt(t('converter.amount')) || '0');
    const margin = parseFloat(prompt(t('converter.margin')) || '0');
    if (cost && margin) {
  ...
  return (
    <div className="p-4">
      ...
      <div className="bg-white dark:bg-koko-blue rounded-2xl p-5 shadow-koko space-y-4">
        <input type="number" placeholder={t('converter.amount')} value={amount} onChange={e => setAmount(e.target.value)} className="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white" />
        ...
        <button onClick={handleConvert} className="w-full py-3 bg-koko-orange text-white font-bold rounded-xl hover:bg-koko-orange-dark transition-colors duration-200">
          {t('converter.convert')}
        </button>
        {result !== null && (
        ...
        <hr className="my-4" />
        <button onClick={handleMargin} className="w-full py-3 border-2 border-koko-orange text-koko-orange font-bold rounded-xl hover:bg-koko-orange/10 transition-colors duration-200">
          {t('converter.margin')}
        </button>
