  ...
  return (
    <div className="p-4" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <Link
        ...
        className="flex items-center justify-center w-full py-3 mb-4 border-2 border-dashed border-koko-orange text-koko-orange font-bold rounded-xl hover:bg-koko-orange/5 transition-colors"
      >
      ...
      <div className="bg-white dark:bg-koko-blue rounded-2xl p-5 shadow-koko mb-4">
        <div className="flex justify-between items-center mb-3">
          ...
          <button
            ...
            className="flex items-center text-koko-orange font-medium"
          >
        ...
        {medications.map(med => (
          <div key={med.id} className="border-b border-gray-100 dark:border-gray-700 py-3 last:border-0">
            <div className="flex justify-between items-start">
              ...
              <button onClick={() => handleDelete(med.id)} className="text-red-500 ml-2">
                <Trash2 size={18} />
              </button>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-koko-blue p-5 rounded-2xl shadow-lg w-full max-w-md mx-4">
            ...
            <form onSubmit={handleAddMed} className="space-y-3">
                ...
                {newMed.times.map((time, idx) => (
                  <div key={idx} className="flex items-center gap-2 mb-2">
                    ...
                    {newMed.times.length > 1 && (
                      <button type="button" onClick={() => removeTime(idx)} className="text-red-500">
                        <Trash2 size={16} />
                      </button>
                    )}
                  </div>
                ))}
                <button type="button" onClick={addTime} className="text-sm text-koko-orange">
                  + Ajouter une heure
                </button>
              </div>
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setShowAdd(false)} className="flex-1 py-2 border rounded-lg">
                  Annuler
                </button>
